window.rtrlApp.review = (function() {
    ...
    function renderModal() {
            ...
            <div class="review-window">
                <div class="review-header">
                    <div class="header-top-row">
                        <h3 style="margin:0; font-size:1.2rem; color:#1e293b; font-weight:800;">${title}</h3>
                    </div>
        ...
        document.getElementById('rev-only-active').onclick = () => { masterData.forEach(d => d._checked = d._reviewStatus === 'Active'); activeFilters = {search:'', contact:'all', rating:0, ownerType:'all'}; refreshUI(); debouncedSave(); };
    ...
    function renderToolbarState() {
        ...
        document.querySelectorAll('.review-table th[data-sort]').forEach(th => { th.classList.toggle('sort-active', th.dataset.sort === currentSort.key); const icon = th.querySelector('i'); if (th.dataset.sort === currentSort.key) { icon.className = currentSort.dir === 'asc' ? 'fas fa-sort-up' : 'fas fa-sort-down'; } else { icon.className = 'fas fa-sort'; } });
    ...
    function mapToMaster(sel) { return sel.map(i => ({ "BusinessName": i.BusinessName, "Category": i.Category, "Suburb/Area": i.Suburb, "StreetAddress": i.StreetAddress, "Website": createLink(i.Website), "OwnerName": i.OwnerName, "Email 1": i.Email1, "Email 2": i.Email2, "Email 3": i.Email3, "Phone": i.Phone, "InstagramURL": createLink(i.InstagramURL), "FacebookURL": createLink(i.FacebookURL), "GoogleMapsURL": createLink(i.GoogleMapsURL), "StarRating": i.StarRating, "ReviewCount": i.ReviewCount })); }
    function exportMaster() { const sel = masterData.filter(d => d._checked); if (!sel.length) return alert("Select leads first."); const ws = XLSX.utils.json_to_sheet(mapToMaster(sel)); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Business List (Unique)"); XLSX.writeFile(wb, `Refined_Full_DuplicatesRemoved_${currentJobId}.xlsx`); }
    async function exportZip() {
        const sel = masterData.filter(d => d._checked); if (!sel.length) return alert("Select leads first.");
        const zip = new JSZip(); const wsFull = XLSX.utils.json_to_sheet(mapToMaster(sel)); const wbFull = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wbFull, wsFull, "Business List (Unique)");
        zip.file(`Refined_Full_DuplicatesRemoved_${currentJobId}.xlsx`, XLSX.write(wbFull, {type:'buffer', bookType:'xlsx'}));
        const sms = sel.filter(b => b.Phone && b.Phone.startsWith("614")).map(b => { let fn = "", ln = ""; if (b.OwnerName && b.OwnerName.trim() !== "") { const p = b.OwnerName.trim().split(" "); fn = p.shift(); ln = p.join(" "); } return { FirstName: fn, LastName: ln, Organization: b.BusinessName || "", Email: b.Email1 || "", FaxNumber: "", MobileNumber: b.Phone || "", CustomField1: b.Category || "", CustomField2: b.Suburb || "", CustomField3: "", CustomField4: "", Unsubscribed: "" }; });
        ...
        const con = sel.filter(d => d.Email1 || d.Email2 || d.Email3).map(d => { let st = ''; if (d.StreetAddress) { const m = d.StreetAddress.match(/\b([A-Z]{2,3})\b(?= \d{4,})/); st = m ? m[1] : ''; } return { "Company": d.BusinessName || '', "Address_Suburb": d.Suburb || '', "Address_State": st, "Notes": `Refined_Search_${currentJobId}`, "Category": d.Category || '', "email_1": d.Email1 || '', "email_2": d.Email2 || '', "email_3": d.Email3 || '', "facebook": d.FacebookURL || '', "instagram": d.InstagramURL || '', "linkedin": '', }; });
