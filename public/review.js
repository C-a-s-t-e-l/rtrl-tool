window.rtrlApp.review = (function () {
  ...
  async function openReview(jobId) {
    currentJobId = jobId;
    try {
      const { data, error } = await getSbClient()
        .from("jobs")
  ...
  function processData(results, exclusionList) {
    ...
    masterData = results.map((item, index) => {
      ...
      return {
        ...
        ManualNotes: item.ManualNotes || "" // Ensure notes property exists
      };
    });
    applyFiltersAndSort();
  }
  ...
  function applyFiltersAndSort() {
    ...
    if (activeFilters.contact === "mobile")
      data = data.filter(d => (d.Phone || "").startsWith("614") || (d.Phone || "").startsWith("04"));
    else if (activeFilters.contact === "email")
      data = data.filter(d => d.Email1);
    else if (activeFilters.contact === "both")
      data = data.filter(d => d.Email1 && ((d.Phone || "").startsWith("614") || (d.Phone || "").startsWith("04")));

    if (activeFilters.rating > 0)
      data = data.filter(d => parseFloat(d.StarRating || 0) >= activeFilters.rating);
  ...
  function renderModal() {
    ...
    const title = `${s.customCategory || s.primaryCategory || "Search"} in ${s.area || "Area"}${jobParams.radiusKm ? ` (${jobParams.radiusKm}km Radius)` : ""}`.replace(/"/g, "");
    ...
    overlay.innerHTML = `
            <div class="review-window">
                <div class="review-header">
                    ...
                    <div class="header-bottom-row">
                        <div class="review-summary">
                            ...
                            <button class="btn-smart-clean" id="rev-smart-clean" title="Uncheck leads with no contact info" style="margin-left:10px">Clean Junk Leads</button>
                            <span class="sum-pill sum-active" style="margin-left:10px">${masterData.filter(d => d._reviewStatus === "Active").length} Unique</span>
                        ...
                        <div class="review-search-wrapper">
                            <input type="text" class="review-search" placeholder="Search by name, category, suburb..." id="rev-search" value="${activeFilters.search}">
                            <button class="btn-review-close" id="rev-only-active">Reset to Active Only</button>
                        </div>
                    </div>
                </div>
                <div class="review-toolbar">
                    ...
                    <div class="filter-group-inline">
                        ...
                        <select class="filter-select" id="rev-filter-rating">
                            ...
                            ${[4.5, 4, 3.5, 3].map((v) => `<option value="${v}" ${activeFilters.rating === v ? "selected" : ""}>${v}+ Stars</option>`).join("")}
                        </select>
                    </div>
                    <div style="width:1px; height:20px; background:#e2e8f0;"></div>
                    <div class="filter-group-inline">
                ...
                <div class="review-footer">
                    ...
                    <div class="tooltip-wrapper"><button class="btn-review-export" style="background:#10b981" id="rev-xlsx">Refined Masterlist (.xlsx)</button><span class="tooltip-text"><b>Full Detail Export</b>Includes your manual corrections and Internal Notes.</span></div>
                    <div class="tooltip-wrapper"><button class="btn-review-export" id="rev-zip">Refined Full File (.zip)</button><span class="tooltip-text"><b>Complete Package</b>Cleans all sub-files (SMS/Email lists) based on your selection.</span></div>
                </div>
            </div>`;
    ...
    document.getElementById("rev-smart-clean").onclick = () => {
        ...
        alert(`Unchecked ${count} leads that had no contact info.`);
    };
