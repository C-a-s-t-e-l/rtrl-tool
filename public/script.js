      ...
      if (update.status) {
        ...
        if (update.status === "completed" || update.status === "failed") {
          ...
          if (update.status === "completed") {
            ...
            if (card) card.className = "status-card phase-error";
            if (icon) icon.className = "fas fa-times-circle";
            if (headline) headline.textContent = "Job Failed";
          }
        }
      }
    });
...
supabaseClient.auth.onAuthStateChange(async (event, session) => {
    ...
    async function saveCurrentPostcodeList() {
      ...
      const listName = prompt(
        "Please enter a name for this postcode list:",
        ""
      );
        ...
        geocoder.geocode(request, (results, status) => {
          if (status === google.maps.GeocoderStatus.OK && results[0]) {
            ...
            if (
              ...
              const suburbName = suburbComponent
