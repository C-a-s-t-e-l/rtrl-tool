...
if (!GOOGLE_MAPS_API_KEY || !BRIGHTDATA_API_TOKEN) {
  console.error(
    "ERROR: MAPS_API_KEY or BRIGHTDATA_API_TOKEN not found in .env file!"
  );
  process.exit(1);
}
  ...
  socket.on(
    ...
    async ({
      ...
      if (!areaQuery || !country)
        return socket.emit("scrape_error", {
          error: `Missing location or country data.`,
        });

      const searchQuery = isIndividualSearch
      ...
      try {
        ...
        if (useProxy) {
          ...
          socket.emit(
            "log",
            `[Server] Using Bright Data proxy server for detail scraping.`
          );
        }
        ...
        socket.emit(
          "log",
          `[Server] Starting URL collection phase via Bright Data API...`
        );
        ...
        socket.emit(
          "log",
          `-> URL Collection complete. Discovered ${allDiscoveredUrls.size} unique listings. Now processing...`
        );

        if (allDiscoveredUrls.size === 0) {
          socket.emit(
            "log",
            "No business URLs were found. The scrape cannot proceed. This might be due to a change in Google Maps page structure.",
            "error"
          );
        }
        ...
        for (let i = 0; i < urlList.length; i += CONCURRENCY) {
          ...
          const promises = batch.map(async (urlToProcess) => {
            ...
            try {
              ...
              fullBusinessData.Category = isIndividualSearch
...
async function collectGoogleMapsUrlsContinuously(searchQuery, socket) {
  socket.emit(
    "log",
    "   -> Using Bright Data Web Unlocker API to collect URLs..."
  );
  try {
    const urlToScrape = `https://www.google.com/maps/search/${encodeURIComponent(
      searchQuery
    )}`;
    ...
    const scriptContentMatch = htmlContent.match(
    ...
    if (!scriptContentMatch || !scriptContentMatch[1]) {
      socket.emit(
        "log",
        "   -> Could not find the embedded data script. Google may have changed its structure.",
        "error"
      );
      return [];
    }
    ...
    try {
      ...
      socket.emit(
        "log",
        `   -> Failed to parse embedded JavaScript object: ${e.message}`,
        "error"
      );
    ...
    if (data?.[0]?.[1]) {
      for (const component of data[0][1]) {
        ...
        if (
          potentialResults &&
          Array.isArray(potentialResults) &&
          potentialResults.some((r) => r?.[6]?.[43])
        ) {
    ...
    socket.emit(
      "log",
      `   -> Intelligent parser found ${uniqueUrls.length} unique URLs from embedded data.`
    );
    return uniqueUrls;
  } catch (error) {
    ...
    socket.emit(
      "log",
      `CRITICAL ERROR: Bright Data API call failed. Details: ${error.message}`,
      "error"
    );
...
async function scrapeWebsiteForGoldData(page, websiteUrl, socket) {
  ...
  try {
    ...
    socket.emit(
      "log",
      `   -> Could not scrape ${websiteUrl}. Error: ${
        error.message.split("\n")[0]
      }`
    );
  }
